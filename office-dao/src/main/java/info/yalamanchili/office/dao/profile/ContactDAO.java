...
public class ContactDAO implements AbstractDao<Contact> {
	...
	// @Override
	public List<Contact> query(int start, int limit) {
		String query = "from " + Contact.class.getCanonicalName();
	...
	public Long size() {
		String query = "select count(entity) from "
				+ Contact.class.getCanonicalName() + " entity";
		Query getEntitiesSize = em.createQuery(query);
		return (Long) getEntitiesSize.getSingleResult();
	}

	public List<String> getSuggestionsForName(String name, Contact entity) {
