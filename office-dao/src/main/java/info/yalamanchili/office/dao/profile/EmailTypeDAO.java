...
@Stateless
public class EmailTypeDAO implements AbstractDao<EmailType> {

	protected EntityManager em;

	// @Override
	public EmailType findById(Long id) {
		return em.find(EmailType.class, id);
	}

	// @Override
	public void save(EmailType entity) {
		em.merge(entity);
	}
	...
	// @Override
	public List<EmailType> query(int start, int limit) {
		String query = "from " + EmailType.class.getCanonicalName();
	...
	public Long size() {
		String query = "select count(entity) from "
				+ EmailType.class.getCanonicalName() + " entity";
		Query getEntitiesSize = em.createQuery(query);
		return (Long) getEntitiesSize.getSingleResult();
	}

	public List<String> getSuggestionsForName(String name, EmailType entity) {
