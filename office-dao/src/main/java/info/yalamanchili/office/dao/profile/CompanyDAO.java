...
public class CompanyDAO implements AbstractDao<Company> {
	...
	// @Override
	public List<Company> query(int start, int limit) {
		String query = "from " + Company.class.getCanonicalName();
	...
	public Long size() {
		String query = "select count(entity) from "
				+ Company.class.getCanonicalName() + " entity";
		Query getEntitiesSize = em.createQuery(query);
		return (Long) getEntitiesSize.getSingleResult();
	}

	public List<String> getSuggestionsForName(String name, Company entity) {
