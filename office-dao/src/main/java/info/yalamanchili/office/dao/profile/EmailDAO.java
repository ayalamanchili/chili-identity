...
public class EmailDAO implements AbstractDao<Email> {
	...
	// @Override
	public List<Email> query(int start, int limit) {
		String query = "from " + Email.class.getCanonicalName();
	...
	public Long size() {
		String query = "select count(entity) from "
				+ Email.class.getCanonicalName() + " entity";
		Query getEntitiesSize = em.createQuery(query);
		return (Long) getEntitiesSize.getSingleResult();
	}

	public List<String> getSuggestionsForName(String name, Email entity) {
