...
public class EmployeeDAO implements AbstractDao<Employee> {
	...
	public List<Employee> query(int start, int limit) {
		String query = "from " + Employee.class.getCanonicalName();
	...
	public Long size() {
		String query = "select count(entity) from "
				+ Employee.class.getCanonicalName() + " entity";
		Query getEntitiesSize = em.createQuery(query);
		return (Long) getEntitiesSize.getSingleResult();
	}

	public List<String> getSuggestionsForName(String name, Employee entity) {
