...
import info.yalamanchili.office.client.reports.recruiting.SkillSetSearchPanel;
...
public class ReportsMenu extends Composite {
    ...
    protected void configureReportsMenu() {
        ...
        if (Auth.hasAnyOfRoles(ROLE.ROLE_RECRUITER)) {
            reportsMenuBar.addItem("Recruiting", recruitingMaintainenceCmd);
        }
    ...
    Command recruitingMaintainenceCmd = new Command() {
        @Override
        public void execute() {
            TabPanel.instance().getReportingPanel().entityPanel.clear();
            TabPanel.instance().getReportingPanel().sidePanelTop.clear();
            TabPanel.instance().getReportingPanel().sidePanelTop.add(new SkillSetSearchPanel());
        }
    };
}
