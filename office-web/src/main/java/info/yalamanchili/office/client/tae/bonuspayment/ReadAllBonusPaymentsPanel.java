...
//        TabPanel.instance().timeandExpensePanel.entityPanel.add(new UpdateBonusPaymentsPanel(bonusPaymentsId, getEntity(entityId)));
    ...
    public void fillData(JSONArray entities) {
        for (int i = 1; i <= entities.size(); i++) {
            ...
            table.setText(i, 4, DateUtils.getFormatedDate(JSONUtils.toString(entity, "paidDate"), DateTimeFormat.PredefinedFormat.DATE_LONG));
    ...
    protected void addOptionsWidget(int row, JSONObject entity) {
        if (Auth.hasAnyOfRoles(Auth.ROLE.ROLE_ADMIN, Auth.ROLE.ROLE_TIME)) {
