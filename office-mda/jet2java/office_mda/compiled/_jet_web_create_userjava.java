...
public class _jet_web_create_userjava implements JET2Template {
    private static final String _jetns_f = "org.eclipse.jet.formatTags"; //$NON-NLS-1$
    private static final String _jetns_java = "org.eclipse.jet.javaTags"; //$NON-NLS-1$

    public _jet_web_create_userjava() {
        super();
    }
    ...
    public void generate(final JET2Context context, final JET2Writer __out) {
        ...
        while (_jettag_java_format_2_1.okToProcessBody()) {
            ...
            while (_jettag_f_bundle_3_1.okToProcessBody()) {
                ...
                out.write(NL);         
                out.write("import info.yalamanchili.gwt.widgets.CreateUserAbstract;");  //$NON-NLS-1$        
                out.write(NL);         
                ...
                out.write(NL);         
                out.write("public class CreateUserPanel extends CreateUserAbstract {");  //$NON-NLS-1$        
                out.write(NL);         
                out.write("\t");  //$NON-NLS-1$        
                ...
                out.write(NL);         
                out.write("\tpublic void createUserClicked() {");  //$NON-NLS-1$        
                out.write(NL);         
                out.write("\tfinal YUser yuser = new YUser();");  //$NON-NLS-1$        
                out.write(NL);         
                out.write("\t\tfinal YUser user = new YUser();");  //$NON-NLS-1$        
                out.write(NL);         
                out.write("\t\tuser.setUsername(usernameTF.getText());");  //$NON-NLS-1$        
                out.write(NL);         
                out.write("\t\tuser.setPasswordHash(passwordTF.getPassword());");  //$NON-NLS-1$        
                out.write(NL);         
                out.write("\t\tSet<String> roles = new HashSet<String>();");  //$NON-NLS-1$        
                out.write(NL);         
                out.write("\t\troles.add(\"user\");");  //$NON-NLS-1$        
                out.write(NL);         
                out.write("\t\tAdminServiceAsync.instance().createUser(user, roles,");  //$NON-NLS-1$        
                out.write(NL);         
                out.write("\t\t\t\tnew ALAsyncCallback<Void>() {");  //$NON-NLS-1$        
                out.write(NL);         
                out.write("\t\t\t\t\t@Override");  //$NON-NLS-1$        
                out.write(NL);         
                out.write("\t\t\t\t\tpublic void onResponse(Void arg0) {");  //$NON-NLS-1$        
                out.write(NL);         
                out.write("\t\t\t\t\t\tnew ResponseStatusWidget().show(\"new User created\");");  //$NON-NLS-1$        
                out.write(NL);         
                out.write("\t\t\t\t\t\tCreateUserPanel.this.hide();");  //$NON-NLS-1$        
                out.write(NL);         
                out.write("\t\t\t\t\t}");  //$NON-NLS-1$        
                out.write(NL);         
                out.write(NL);         
