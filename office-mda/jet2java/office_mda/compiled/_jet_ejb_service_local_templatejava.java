...
public class _jet_ejb_service_local_templatejava implements JET2Template {
    private static final String _jetns_f = "org.eclipse.jet.formatTags"; //$NON-NLS-1$
    private static final String _jetns_java = "org.eclipse.jet.javaTags"; //$NON-NLS-1$

    public _jet_ejb_service_local_templatejava() {
        super();
    }
    ...
    private static final TagInfo _td_f_message_9_1 = new TagInfo("f:message", //$NON-NLS-1$
            9, 1,
            new String[] {
            },
            new String[] {
    ...
    public void generate(final JET2Context context, final JET2Writer __out) {
        ...
        while (_jettag_java_format_2_1.okToProcessBody()) {
            ...
            while (_jettag_f_bundle_3_1.okToProcessBody()) {
                ...
                out.write(";");  //$NON-NLS-1$        
                out.write(NL);         
                out.write("import javax.ejb.Local;");  //$NON-NLS-1$        
                out.write(NL);         
                out.write(NL);         
                RuntimeTagElement _jettag_f_message_9_1 = context.getTagFactory().createRuntimeTag(_jetns_f, "message", "f:message", _td_f_message_9_1); //$NON-NLS-1$ //$NON-NLS-2$
                _jettag_f_message_9_1.setRuntimeParent(_jettag_f_bundle_3_1);
                _jettag_f_message_9_1.setTagInfo(_td_f_message_9_1);
                _jettag_f_message_9_1.doStart(context, out);
                JET2Writer _jettag_f_message_9_1_saved_out = out;
                while (_jettag_f_message_9_1.okToProcessBody()) {
                    out = out.newNestedContentWriter();
                    out.write("generated.class.comment");  //$NON-NLS-1$        
                    _jettag_f_message_9_1.handleBodyContent(out);
                }
                out = _jettag_f_message_9_1_saved_out;
                _jettag_f_message_9_1.doEnd();
                out.write(NL);         
                out.write("@Local");  //$NON-NLS-1$        
                out.write(NL);         
                out.write("public interface ");  //$NON-NLS-1$        
                out.write(context.embeddedExpressionAsString("${$class/@name}", 11, 18)); //$NON-NLS-1$ //$NON-NLS-2$
                out.write("ServiceLocal extends  ");  //$NON-NLS-1$        
                out.write(context.embeddedExpressionAsString("${$class/@name}", 11, 55)); //$NON-NLS-1$ //$NON-NLS-2$
